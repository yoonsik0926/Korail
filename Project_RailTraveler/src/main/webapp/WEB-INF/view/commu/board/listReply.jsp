...
<ul class="cmlist" id="cmt_list">
	<c:forEach var="vo" items="${listReply}">
		...
		<li ${vo.answer==0?'class=""':'class="reply"'}
			${vo.userId ==sessionScope.member.userId? 'style="background: #eee"':''}>

			<c:if test="${vo.answer!=0}">
				<div style="float: left; margin-right: 10px;">
					<i class="fas fa-reply" style="transform: rotate(180deg);"></i>&nbsp;
				</div>
			</c:if> <c:if test="${empty vo.deleteId}">
				<div class="comm_cont">
					<div class="h">
							...
							<table role="presentation" cellspacing="0">
								<tbody>
									<tr>
										...
										</a> <c:if test="${writer==vo.userId}">
												<span
													style="position: relative; display: inline-block; width: 41px; height: 20px; margin: 0 8px 0 -2px; border: 1px solid; border-radius: 8px; font-size: 11px; font-weight: normal; letter-spacing: -1px; text-align: center; text-indent: -1px; cursor: pointer; top: -2; line-height: 20px; color: #ff2f3b;">작성자</span>
											</c:if></td>
									</tr>
								</tbody>
							</table>
						</div>
						<span class="date m-tcol-c filter-50"
							style="color: #999; top: 3px; font-size: 12px; position: relative;">${vo.created}</span>
						<c:if test="${vo.answer==0}">
							<span class="dsc_comm"><a
								class="m-tcol-c btnReReply${vo.boardReplyNum}"
								style="cursor: pointer; font-size: 10px;"
								onclick="displayReplyForm(${vo.boardReplyNum});">답글</a> </span>
						</c:if>
							<p class="btn_edit m-tcol-c">
						<c:if test="${vo.userId ==sessionScope.member.userId}">
								<a class="filter-70 m-tcol-c _btnEdit btnUpdateReply${vo.boardReplyNum}"
									style="color: #999; cursor: pointer; font-size: 11px;"
									onclick="displayReplyUpdateForm(${vo.boardReplyNum},'${vo.content}');"
									>수정</a><span
									class="filter-30 m-tcol-c" style="">|</span></c:if>
									<c:if
							test="${vo.userId ==sessionScope.member.userId or 'admin'==sessionScope.member.userId}">
									<a class="filter-70 m-tcol-c _btnDelete deleteReply"
									data-replyNum="${vo.boardReplyNum}" data-pageNo="${pageNo}"
									style="color: #999; cursor: pointer; font-size: 11px;" 
									>삭제</a></c:if>
							</p>
					...
					<p class="comm m-tcol-c" style="padding-left: 27px;">
						<span class="comm_body replyContent${vo.boardReplyNum}" style="color: #999; font-size: 14px;">${vo.content}</span>
					<span class="replyUpdateForm${vo.boardReplyNum}" style="display: none;">
						...
						<span
							...
							<button type='button' class='btn btn-default'
								...
								onclick="updateReply(${vo.boardReplyNum},${pageNo});">수정</button>
			...
			</c:if> <c:if test="${not empty vo.deleteId}">
		...
		<div class="replyForm${vo.boardReplyNum}" style="display: none;">
			<li>
				<div style="float: left; margin-right: 10px;">
					<i class="fas fa-reply" style="transform: rotate(180deg);"></i>&nbsp;
				</div>
				<table class="cminput" style="width: 96%; float: left;">
					<tbody>
						<tr>
							<td><textarea class='boxTA'
									style='width: 92%; height: 100px; float: left; resize: none; overflow-y: scroll;'
								...
								<div
									...
									<button type='button' class='btn btn-default'
										data-num='${vo.boardReplyNum}' data-page="${pageNo}"
										style='width: 100%; height: 90px; padding: 2px 1px;'
										onclick="insertReply(${vo.boardReplyNum},${pageNo});">등록</button>

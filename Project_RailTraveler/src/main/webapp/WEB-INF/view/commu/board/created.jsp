...
function deleteFile(num) {
	...
	var url="<%=cp%>/board/deleteFile";
	var query="boardFileNum="+num;
			...
			<table class="tb-created">
				...
				<c:if test="${mode=='update'}">
				   <c:forEach var="vo" items="${listFile}">
						  <tr id="f${vo.boardFileNum}" height="40" style="border-bottom: 1px solid #cccccc;"> 
						      <td class="tb-title" width="100" style="text-align: center;">첨부된파일</td>
						      <td style="padding-left:10px;">
						        <a href="javascript:deleteFile('${vo.boardFileNum}');"><i class="far fa-trash-alt"></i></a> 
								${vo.originalFilename}
						      </td>
						  </tr>
				   </c:forEach>
				...
				<tr>
					<td>
						<button type="button" class="btn btn-default"
							...
							onclick="javascript:location.href='<%=cp%>/board/board';">${mode=='update'?'수정취소':'등록취소'}</button>
